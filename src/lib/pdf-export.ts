...
/**
 * Logo'yu base64 formatına çevirir (PDF'e gömülmek için).
 */
...
/**
 * Proje ölçüm sonuçlarını PDF rapor olarak indirir.
 *
 * Rapor Formatı:
 * - Sol üst: ESOGÜ logosu
 * - Sağ üst: Tarih
 * - Orta: Üniversite / Bölüm / Analiz Laboratuvarı başlığı
 * - Tablo: Tüm ölçümler
 */
export async function exportProjectPDF(project: Project): Promise<void> {
  ...
  doc.setFontSize(11);
  doc.setTextColor(30, 41, 82);
  doc.text('Kirec (CaCO3) Tayini Raporu', pageWidth / 2, 30, { align: 'center' });
  ...
  const rows = project.measurements.map((m) => {
    ...
    const dStr = d.toLocaleDateString('tr-TR') + ' ' + d.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
    return [
      r.ornekNo,
      dStr,
      r.agirlik.toString(),
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      1: { halign: 'center' },
      8: { fontStyle: 'bold', textColor: [74, 124, 222] },
      9: { halign: 'left' },
    },
    ...
    didDrawPage: (data) => {
      ...
      // Sonraki sayfa için header tekrarı
      if (data.pageNumber > 1) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        doc.setTextColor(30, 41, 82);
        doc.text('Kirec (CaCO3) Tayini Raporu', 14, 10);
  ...
  doc.save(`${safeName}_CaCO3_Raporu.pdf`);
}
